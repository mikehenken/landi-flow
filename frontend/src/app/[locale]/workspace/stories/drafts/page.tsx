  ...
  return (
    ...
    >
      <div className="flex h-full min-h-0 flex-col" data-testid="story-drafts-view" data-cap="CAP-005">
        {drafts.length === 0 ? (
          <p className="p-6 text-sm text-muted-foreground">No drafts — create a Story and save as draft.</p>
        ) : (
          <>
            <div className="border-b border-border px-6 py-3">
              <p className="text-sm text-muted-foreground">
                {drafts.length} draft{drafts.length === 1 ? '' : 's'} — publish to add to the team board.
              </p>
            </div>
            <StoryListView
              stories={drafts}
