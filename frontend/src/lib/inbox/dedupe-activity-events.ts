...
/**
 * Stable identity for signal.attached rows so retries / agent loops collapse.
 * Prefers explicit signal id, then correlation, then kind+status+source fingerprint.
 */
...
/**
 * Dedupe activity feed rows. For signal.attached, collapse by
 * (story_id, signal identity, type). Non-signal events pass through by id.
 */
